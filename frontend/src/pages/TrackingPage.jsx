...
const TrackingPage = () => {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6 relative">
      ...
      {delivered && (
        <div className="bg-green-100 border border-green-200 text-green-800 px-4 py-3 rounded-lg mb-6 flex items-center shadow-sm">
      ...
      <div className="bg-gray-100 rounded-2xl h-[280px] w-full mb-8 relative overflow-hidden shadow-inner border border-gray-200">
        {/* Simple map background using SVG */}
        <svg className="absolute inset-0 w-full h-full" xmlns="http://www.w3.org/2000/svg">
          {/* Streets Grid */}
          <path d="M0 60 H800 M0 140 H800 M0 220 H800 M100 0 V300 M260 0 V300 M420 0 V300 M580 0 V300" stroke="#E5E7EB" strokeWidth="15" fill="none" />
        ...
        {/* Labels */}
        <span className="absolute top-2 left-[180px] text-[10px] text-gray-400 font-bold uppercase rotate-90">CG Road</span>
        <span className="absolute top-[80px] left-[20px] text-[10px] text-gray-400 font-bold uppercase">Maninagar</span>
        ...
        <div className="absolute top-[40px] left-[40px] w-8 h-8 bg-white border-2 border-[#E8521A] rounded-full flex items-center justify-center shadow-md transform -translate-x-1/2 -translate-y-1/2 z-10">
          ...
          <div className="absolute top-10 whitespace-nowrap text-xs font-bold bg-white px-2 py-0.5 rounded shadow-sm">Your Kitchen</div>
        </div>
        ...
        <div className="absolute top-[180px] left-[300px] w-8 h-8 bg-white border-2 border-blue-500 rounded-full flex items-center justify-center shadow-md transform -translate-x-1/2 -translate-y-1/2 z-10">
          ...
          <div className="absolute top-10 whitespace-nowrap text-xs font-bold bg-white px-2 py-0.5 rounded shadow-sm">Your Home</div>
        </div>
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between relative">
          ...
          <div className="flex flex-col items-center">
            ...
            <span className="text-[10px] font-semibold text-gray-700 text-center w-16">Confirmed</span>
          </div>
          <div className="flex flex-col items-center">
            <div className="w-8 h-8 bg-green-500 rounded-full flex items-center justify-center text-white shadow-sm mb-2 text-sm font-bold">✓</div>
            <span className="text-[10px] font-semibold text-gray-700 text-center w-16">Preparing</span>
          </div>
          <div className="flex flex-col items-center">
            {delivered ? (
              <div className="w-8 h-8 bg-green-500 rounded-full flex items-center justify-center text-white shadow-sm mb-2 text-sm font-bold">✓</div>
            ) : (
              <div className="w-8 h-8 bg-[#E8521A] border-4 border-orange-100 rounded-full shadow-sm mb-2 relative flex items-center justify-center">
                <div className="w-2 h-2 bg-white rounded-full animate-ping"></div>
              </div>
            )}
            <span className={`text-[10px] font-semibold text-center w-16 ${delivered ? 'text-gray-700' : 'text-[#E8521A]'}`}>Out for Delivery</span>
          </div>
          <div className="flex flex-col items-center">
            {delivered ? (
              <div className="w-8 h-8 bg-green-500 rounded-full flex items-center justify-center text-white shadow-sm mb-2 text-sm font-bold">✓</div>
            ) : (
              <div className="w-8 h-8 bg-gray-200 rounded-full flex items-center justify-center text-gray-500 shadow-sm mb-2 text-sm">⏱</div>
            )}
            <span className="text-[10px] font-semibold text-gray-500 text-center w-16">Delivered</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 flex items-center justify-between mb-6">
        <div className="flex items-center">
          ...
          <div className="ml-4">
            ...
            <p className="text-xs text-gray-500">⭐ 4.8 &middot; {delivered ? '0 km away' : '1.2 km away'} &middot; {delivered ? 'Arrived' : '~8 min'}</p>
          </div>
        </div>
        <button 
          onClick={() => alert("Calling Delivery Agent...")}
          className="w-10 h-10 bg-orange-50 text-[#E8521A] rounded-full flex items-center justify-center hover:bg-orange-100 transition-colors"
        >
          📞
        </button>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        >
          <span className="font-semibold text-gray-800">What's in your tiffin today</span>
          <span className={`transform transition-transform ${itemsOpen ? 'rotate-180' : ''}`}>▼</span>
        </button>
              ...
              <ul className="space-y-3">
                {cartList.map((item, idx) => (
                  <li key={idx} className="flex items-center text-sm">
                    <div className="w-6 h-6 bg-orange-100 text-[#E8521A] rounded flex items-center justify-center font-bold text-xs mr-3">
                      {item.quantity}x
                    </div>
                    <span className="font-medium text-gray-700">{item.meal.name}</span>
                  </li>
                ))}
              </ul>
            ) : (
              <p className="text-gray-500 text-sm">Dummy Tiffin Order</p>
            )}
          </div>
        )}
      </div>
